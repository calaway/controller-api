...
async function create(pg_pool, req, res){
  ...
  let {topic: topic_uuid} = await common.topic_exists(pg_pool, payload.topic, payload.region);
  let {app_uuid, app_name, space_name} = await common.app_exists(pg_pool, payload.app);

  // Create in DB
  await insert_acl(pg_pool, [uuid.v4(), topic_uuid, app_uuid, payload.role]);

  // In test mode, don't actually create the ACL.
  if (!process.env.TEST_MODE) {
    await common.alamo.topic_acls.create(payload.region, payload.cluster, payload.topic, app_name, space_name, payload.role);
  }
