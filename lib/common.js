...
async function check_topic_exists(pg_pool, topic_key, region) {
  let result = await select_topic(pg_pool, [topic_key, region]);
  if (result.length !== 1) {
    throw new http_help.NotFoundError(`The specified topic ${topic_key} does not exist in region ${region}.`);
  }
...
async function check_cluster_exists(pg_pool, cluster_key) {
  if (!/-/.test(cluster_key)){
    throw new http_help.NotFoundError(`Select cluster with UUID or cluster-region syntax.`);
  }
